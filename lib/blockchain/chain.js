...
class Chain extends AsyncEmitter {
  ...
  /*
   * Sync meta tree state.
   */

  async syncTree() {
    const {retargetInterval} = this.network.pow.retargetInterval;
    const last = this.height - (this.height % retargetInterval);
    ...
    for (let height = last + 1; height <= this.height; height++) {
      ...
      await this.db.save(view, height);
    }
  ...
  async getMetaRoot() {
    const interval = this.network.retargetInterval;
